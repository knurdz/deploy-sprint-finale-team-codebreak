...
| T11 |  |  |  |
| T12 |  |  | `ci.yml`'s `build` job already used `actions/setup-node@v4` with `cache: npm` keyed on `team-site/package-lock.json`, and `npm ci` (not `npm install`) — pre-existing from the initial setup. Added `id: setup-node` to that step and a new `Report dependency cache status` step that writes the cache key path, `steps.setup-node.outputs.cache-hit`, and the install command to the job summary, so cache hit/miss is visible evidence on every run instead of buried in raw log lines. `npm ci` and the lockfile-based key are unchanged. |
| T13 |  |  |  |
| T14 |  |  |  |
| T15 |  |  |  |
| T16 |  |  |  |
